...
::html
<section class="hero">
  <p class="slogan">Five deposits, each with a third-party date.</p>
  <h1>Cite</h1>
  ...
  <div class="kpis">
    <div class="kpi"><b>5</b><small>deposited versions, each with its own DOI and a date stamped by a third party</small></div>
...
## Most recent citable version — 3.5.0
...
The library is now at **3.5.1**, a security fix with no API and no format change —
`.s2r` files written by 3.5.0 and 3.5.1 are read by each other. The citable method
did not change, so the reference above still stands; when the 3.5.1 deposit
appears it joins this table. What it fixes is on the
[technical scope page](/scope/).
