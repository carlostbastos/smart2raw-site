...
pt:{
  ...
  d_cross_note:"Repare no que muda entre as duas linhas. Numa coluna que cabe no cache do processador ninguém está limitado por memória: ler 8× menos bytes não compra tempo nenhum, e o caminho compacto chega a perder para o laço direto sobre int64. Conforme a coluna cresce e a forma larga deixa de caber, a razão sobe — e onde exatamente ela cruza 1,00× depende do tamanho do cache desta máquina. É por isso que a economia de espaço vira economia de tempo: não por decreto, mas no ponto em que o dado deixa de caber.",
...
en:{
  ...
  d_cross_note:"Watch what changes between the two rows. On a column that fits in the processor cache nobody is memory-bound: reading 8× fewer bytes buys no time at all, and the compact path can even lose to the direct loop over int64. As the column grows and the wide form stops fitting, the ratio climbs — and exactly where it crosses 1.00× depends on how much cache this machine has. That is why a space saving becomes a time saving: not by decree, but at the point where the data stops fitting.",
